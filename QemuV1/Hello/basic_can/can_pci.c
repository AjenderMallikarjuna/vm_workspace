...
static void receive_message(void __iomem *base, struct can_frame *can)
{
	...
	for(i = 0; i < can->can_dlc -1; i++) {
		printk("[can] data recieved fan speed %02x AC %02x\n",can->data[i],can->data[i+1]);
...
{   
	...
	//printk(KERN_INFO "device_read: Entered.\n");
	receive_message(device->mem_base, &(device->rx_buf[device->rx_in]));
	if (dev->rx_count == 0)
    ...
    printk(KERN_INFO "device_read: Exited.\n");
	return ret;
...
{   
    ...
    printk(KERN_INFO "device_write: Exited.\n");
...
{  
	...
	DPRINTF("device init.\n");
	printk(KERN_INFO "can_pci: Probing device.\n");

	device = kmalloc(sizeof(struct can_dev), GFP_KERNEL);
	if (!device) {
		printk(KERN_ERR "can_pci: Memory allocation failed for 'device'.\n");
		return -ENOMEM;
	}
	...
	{
        printk(KERN_ERR "can_pci: Device revision mismatch.\n");
		return -ENODEV;
	}
	...
	printk(KERN_INFO "can_pci: Registered with major number %d\n", device_major);
	if (result < 0) {
...
{  
	printk(KERN_INFO "can_pci: Module initialization started.\n");
